...
class TestArticlesViews:
    async def test_can_fetch_article_by_id(self, fake_uow: AbstractUnitOfWork) -> None:
        ...
        assert articles[0] == await views.articles.fetch_article_by_id(article_id=1, uow=fake_uow)
        assert articles[1] == await views.articles.fetch_article_by_id(article_id=2, uow=fake_uow)
